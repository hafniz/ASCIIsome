    ...
    public static class Decomposer
    {
        ...
        private static void ProcessFileTail(FileStream fileStream)
        {
            while (true)
            {
                ...
                if (fileStream.ReadByte() == 0) // Read the last byte of the file. 
                {
                    fileStream.SetLength(fileStream.Length - 1);
                }
                else
                {
                    fileStream.SetLength(fileStream.Length - 1); // Also delete the 'End of Transmission Block' byte appended during composing process. 
                    break;
                }
            }
        }
